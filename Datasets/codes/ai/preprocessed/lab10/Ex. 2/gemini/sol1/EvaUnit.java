...
public class EvaUnit<T extends Pilot> {
    ...
    public boolean start() {
        ...
        if (pilot instanceof HumanPilot) {
            ...
            if (!humanPilot.canAct()) {
                System.out.println("Human Pilot " + humanPilot.getName() + " has 0 sanity and cannot start.");
                return false;
            }
            humanPilot.reduceSanity(5);
            System.out.println("Human Pilot " + humanPilot.getName() + "'s sanity reduced to " + humanPilot.getSanity());
        }
    ...
    public boolean maneuver(Obstacle obstacle) {
        ...
        if (pilot instanceof HumanPilot) {
            HumanPilot humanPilot = (HumanPilot) pilot;
            if (!humanPilot.canAct()) {
                System.out.println("Human Pilot " + humanPilot.getName() + " has 0 sanity and cannot maneuver.");
                return false;
            }
            ...
            System.out.println("Human Pilot " + humanPilot.getName() + "'s sanity reduced by " + mentalToll + " to " + humanPilot.getSanity() + ". Effective Roll: " + effectiveRoll);
            if (effectiveRoll >= 50) {
