...
public class TicTacToeNXMBlock {
    ...
    public TicTacToeNXMBlock(int rows, int cols, int winCondition) {
        this.rows = rows;
        this.cols = cols;
    ...
    private int[] getAIMove() {
        // Check if Player X has a winning move
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (isValidMove(i, j)) {
                    board[i][j] = 'X';
        ...
        do {
            ...
            attempts++;
            if (attempts > rows * cols * 2) return null; // Should not happen in a non-full board
    ...
    public static void main(String[] args) {
        ...
        new TicTacToeNXMBlock(rows, cols, winCondition).playGame();
        input.close();
    }
}