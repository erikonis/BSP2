...
public class Inventory {
    ...
    public void execute(InventoryOperation operation) {
        operation.execute(this);
    }

    // Internal add method used by AddItemOperation
    boolean addInternal(Item item) {
    ...
    // Internal drop by item method used by DropItemOperationByName
    int dropInternal(Item item) {
    ...
    // Internal drop by index method used by DropItemOperationByIndex
    boolean dropInternal(int index) {
    ...
    // Internal use method used by UseItemOperation
    boolean useInternal(int index) {
        if (index >= 0 && index < items.size()) {
    ...
    // Internal sell method used by SellItemOperation
    boolean sellInternal(int index) {
        if (index >= 0 && index < items.size()) {
            Item item = items.get(index);
