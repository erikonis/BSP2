...
public class PalindromeCaseInsensitive {
    ...
    public static void main(String[] args) {
        ...
        boolean isPal = isPalindromeIgnoreCase(sequence);
        System.out.println(buildResultMessage(sequence, isPal, true));
    }
    ...
    static boolean isPalindromeIgnoreCase(List<Character> chars) {
        int left = 0, right = chars.size() - 1;
        while (left < right) {
            char leftChar = Character.toLowerCase(chars.get(left++));
            char rightChar = Character.toLowerCase(chars.get(right--));
