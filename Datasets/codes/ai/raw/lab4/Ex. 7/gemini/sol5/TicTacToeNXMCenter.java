...
public class TicTacToeNXMCenter {
    ...
    public TicTacToeNXMCenter(int rows, int cols, int winCondition) {
        this.rows = rows;
        this.cols = cols;
    ...
    private int[] getAIMove() {
        ...
        if (rows % 2 != 0 && cols % 2 != 0 && isValidMove(centerRow, centerCol)) {
            return new int[]{centerRow, centerCol};
        }

        // 4. If no winning, blocking, or center move, make a random move
        int row, col;
        int attempts = 0;
        do {
    ...
    public static void main(String[] args) {
        ...
        new TicTacToeNXMCenter(rows, cols, winCondition).playGame();
        input.close();
    }
}