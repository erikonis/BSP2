...
public class Disk {
    ...
    public GameInstalStatus install(Game game_, User user_) {
        if (user_.ownsGameLicense(game_.getLicense())){
            installedSoftware.add(game_);
            return GameInstalStatus.SUCCESS;
        }
        else {
                return GameInstalStatus.NOLICENSE;
         
       }   
    }

    public InstallToolStatus install(Tool tool_, Disk disk_) {
        if (!disk_.getOperatingSystem().equals(OperatingSystem.MACOS)){
            installedSoftware.add(tool_);
            return InstallToolStatus.SUCCESS;
        }
        else {
                return InstallToolStatus.MACOS;
