...
def get_data_loaders():
    ...
    full_train_dataset = datasets.ImageFolder(root=f'data{prefix}/train', transform=transform)
    test_dataset = datasets.ImageFolder(root=f'data{prefix}/test', transform=transform)
...
def train_model(model, device, train_loader, val_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        if not torch.isfinite(loss):
            print(f"Non-finite loss detected at epoch {epoch}. Stopping training to avoid divergence.")
            break
        ...
        for param in model.parameters():
            if torch.isfinite(param.grad).all():
                ...
                print(f"Non-finite gradients at epoch {epoch}.")

    return train_losses, train_accuracies, val_losses, val_accuracies

def plot_metrics(train_accuracies, val_accuracies, train_losses, val_losses, epochs):
...
def evaluate_model(model, test_loader, device):
    ...
    test_accuracy = (np.array(test_preds) == np.array(test_labels)).sum() / len(test_labels) * 100
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=["Human", "AI"], yticklabels=["Human", "AI"], annot_kws={"size": 36} )
...
raw = True
model_name = "rmlp"
model_name_full = "ResMLP-12"
prefix = "raw" if raw else "preprocessed"
output = f"{model_name}_{prefix}/"
...
def main():
    ...
    # Train the model
    train_losses, train_accuracies, val_losses, val_accuracies = train_model(model, device, train_loader, val_loader,
                                                                 epochs)
    print("trained model")

    # Plot metrics
    plot_metrics(train_accuracies, val_accuracies, train_losses, val_losses, epochs)
